...
## `tck cp2k singlepoint-to-deepmd [ROOT] [OUTPUT]`
...
```bash
tck cp2k singlepoint-to-deepmd ./single_points ./cp2k_dataset
```

## `tck cp2k aimd-to-deepmd [ROOT] [OUTPUT]`
...
```bash
tck cp2k aimd-to-deepmd ./aimd ./cp2k_aimd_dataset
```

## 依赖

cp2kdata 已随 TCCKit 默认安装；AIMD 转换无需再单独补装解析器：
