"""Inspect the local TCCKit installation and optional runtime resources."""
...
def doctor(
    ...
    """检查 TCCKit、可选 Python 依赖和 ABACUS 资源目录。"""
    result = inspect_environment()
    if json_output:
        typer.echo(json.dumps(result, ensure_ascii=False, indent=2))
        return

    typer.echo(f"TCCKit       {result['tcckit_version']}")
    typer.echo(f"Python           {result['python']['version']}")
    typer.echo(f"Python 路径      {result['python']['executable']}")
    typer.echo(f"tck 路径         {result['mfk_executable'] or 'PATH 中未找到'}")
    typer.echo("\n可选依赖")
