...
def _require_ase():
    try:
        ...
        raise RuntimeError(
            "缺少 ASE；请安装 TCCKit structure 依赖：pip install 'tcckit[structure]'"
        ) from exc
    return read, write
...
def _require_dpdata():
    try:
        ...
        raise RuntimeError(
            "生成 STRU 需要 dpdata；请安装：pip install 'tcckit[dpdata,structure]'"
        ) from exc
    return dpdata
...
def _require_pymatgen():
    try:
        ...
        raise RuntimeError(
            "缺少 pymatgen；请安装 TCCKit structure 依赖："
            "pip install 'tcckit[structure]'"
        ) from exc
    return CifParser
...
def convert(
    ...
    try:
        ...
        temporary = output.with_name(f".{output.name}.tck-{uuid.uuid4().hex}.tmp")
