...
## `tck gpumd npy-to-xyz DATASET [OUTPUT]`
...
```bash
tck gpumd npy-to-xyz ./deepmd_npy train.xyz
tck gpumd npy-to-xyz ./deepmd_npy train.xyz --virial
```

## `tck gpumd thermo [FILE]`（默认 `thermo.out`）
...
```bash
tck gpumd plot-nep-evaluation ./nep_results
tck gpumd plot-nep-evaluation ./nep_results \
