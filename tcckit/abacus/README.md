...
## `tck abacus check-relax [DIR]`（默认当前目录）
...
## `tck abacus prepare-from-xyz SOURCE TEMPLATE [OUTPUT]`
...
```bash
tck abacus prepare-from-xyz structures.xyz template abacus_tasks
tck abacus prepare-from-xyz xyz_files/ template abacus_tasks
```

## `tck abacus report [ROOT]`
...
## `tck abacus to-deepmd ROOT OUTPUT`
