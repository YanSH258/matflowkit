...
def _require_ase():
    try:
        ...
        raise RuntimeError(
            "缺少 ASE；请安装 TCCT structure 依赖：pip install 'tcct[structure]'"
        ) from exc
    return read, write
...
def _require_dpdata():
    try:
        ...
        raise RuntimeError(
            "生成 STRU 需要 dpdata；请安装：pip install 'tcct[dpdata,structure]'"
        ) from exc
    return dpdata
...
def _require_pymatgen():
    try:
        ...
        raise RuntimeError(
            "缺少 pymatgen；请安装 TCCT structure 依赖："
            "pip install 'tcct[structure]'"
        ) from exc
    return CifParser
...
def convert(
    ...
    try:
        ...
        temporary = output.with_name(f".{output.name}.tcct-{uuid.uuid4().hex}.tmp")
