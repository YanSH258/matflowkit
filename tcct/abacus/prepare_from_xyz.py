...
from tcct.abacus.audit import parse_basis_type, parse_calculation
from tcct.common.io import sha256_file, write_csv, write_json, write_sha256_manifest
from tcct.structure.convert import _require_ase, write_abacus_stru
...
def prepare_from_xyz(
    ...
    staging = output.with_name(f".{output.name}.tcct-{uuid.uuid4().hex}")
    try:
        ...
        for xyz_path in files:
            ...
            for frame_index, atoms in enumerate(frames):
                ...
                suffix = f"tcct_{task_number:06d}"
