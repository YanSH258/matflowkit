...
- `tck abacus prepare-from-xyz`：将多帧 Extended XYZ 或目录中的多个 XYZ 文件
  按帧生成独立的 ABACUS `INPUT/KPT/STRU` 任务，并输出 manifest 和校验值。
- `tck gpumd plot-nep-evaluation`：分别统计和绘制 NEP 训练集、独立测试集的
  energy/force/stress 或 virial 预测误差，并对大数据自动使用密度图。
- `tck deepmd split`：使用固定 seed 的 random 或 uniform 方法划分 DeepMD NPY，
  输出逐帧 manifest、校验值并进行 NPY 回读验证。
- DeepMD 数据集报告可选全帧 PBC 最小原子距离、逐元素对最小距离和阈值计数；
  大型数据集默认不运行该耗时检查。
- `tck cp2k aimd-to-deepmd`：使用 dpdata + CP2KData 将原生 CP2K AIMD
  位置、力、能量、晶胞和可用位力转换为经过回读验证的 DeepMD NPY。
- `tck doctor`：检查 TCCKit 版本、可选依赖和 ABACUS 赝势/轨道目录。
- `tck abacus report`：输出 ABACUS 批量任务的 HTML、JSON、CSV 和 PNG 报告。
...
- `tck abacus check-relax --plot` 合并结构优化检查和收敛作图，移除重复的
  `plot-convergence` 菜单项。
- 交互菜单按“准备输入 → 检查/报告 → 提取数据 → 数据审计”的常用流程重排
  ABACUS、DeepMD 和 GPUMD 子命令编号。
- `tck gpumd plot-nep-evaluation` 改为自动扫描 `loss.out` 及已有的 train/test
...
- 主菜单按 Structure、ABACUS、CP2K、VASP、dpdata、DeepMD、GPUMD、DPA4、System
  排列，并直接显示每个模块的用途。
- `tck cp2k collect` 改为 `tck cp2k singlepoint-to-deepmd`，只输出 DeepMD NPY。
- `tck vasp to-deepmd` 改为 `tck vasp outcar-to-deepmd`。
- `tck gpumd from-deepmd` 改为 `tck gpumd npy-to-xyz`。
