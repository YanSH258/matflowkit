...
def relax(
    ...
    if optimizer not in OPTIMIZER_NAMES:
        typer.secho(
            f"错误: --optimizer 必须为 {', '.join(OPTIMIZER_NAMES)}",
            err=True,
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    ...
    log_path = output.with_name(f"{output.stem}.log")
    trajectory_path = output.with_name(f"{output.stem}_trajectory.extxyz")
    status_path = output.with_name(f"{output.stem}_status.json")
    existing = [
        path
    ...
    try:
        ...
        final_atomic_fmax = maximum_force(
            atoms.get_forces(apply_constraint=True)
        )
