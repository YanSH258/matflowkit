...
Run checks with the exact interpreter that will launch `tcct`:
...
Prefer `python -m deepmd` when the `dp` executable is missing or ambiguous. Confirm that `python`, `tcct`, `deepmd`, PyTorch, ASE, and `dftd3` come from the intended environment.
...
## TCCT model resolution

TCCT resolves the model in this order:
...
The default TCCT calculator loads the model through `deepmd.calculator.DP`. Unless `--no-d3` is supplied, it adds `dftd3.ase.DFTD3(method="pbe", damping="d3bj")`. Therefore default energies, forces, and stresses belong to the combined DPA4 + PBE-D3(BJ) calculator.
