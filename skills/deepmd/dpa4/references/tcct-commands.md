...
Read this reference when choosing a command, constructing inputs, predicting outputs, resuming work, or interpreting exit codes. Confirm details against `tcct dpa4 COMMAND -h` in the active checkout.
...
```bash
tcct dpa4 batch-relax structures.csv
```
...
```bash
tcct dpa4 batch-relax structures.csv --retry-failed --strict
```
